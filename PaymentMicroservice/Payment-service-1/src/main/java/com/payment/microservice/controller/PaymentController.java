...
import com.payment.microservice.service.PaymentUpdateService;
...
public class PaymentController {
   ...
   @Autowired
	...
	public ResponseEntity<PaymentDTO> paymentDetails(@PathVariable String requestId, @PathVariable int cardNumber,
			@PathVariable int creditLimit, @PathVariable int processingCharge,@RequestHeader(name = "Authorization", required = true)String token) throws InvalidTokenException {
	...
	@PostMapping("/reversePayment/{requestId}/{cardNumber}/{creditLimit}/{processingCharge}")
   	public ResponseEntity<PaymentDTO> reversePayment(@PathVariable String requestId, @PathVariable int cardNumber,
  			 @PathVariable int processingCharge,@RequestHeader(name = "Authorization", required = true)String token) throws InvalidTokenException {
         ...
         try {
      	   return new ResponseEntity<>(
      			   paymentServiceImpl.reversePaymentService(requestId, cardNumber, processingCharge),
      			   HttpStatus.OK);
